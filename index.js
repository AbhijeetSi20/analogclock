...
toggler.onclick = function swapStyles() {
    if(styleSheet.getAttribute('href') == main) {
        ...
        toggler.innerHTML = "🕶️"; //dark moon emoji
    }
}
...
const sound = new Audio('jingle-bells-alarm-clock.mp3');
...
alarm.onclick = function setAlarm() {
    ...
    */
    let userHour = prompt("Please enter the hour of which you would like the alarm to be set 😄", "00"); 
    if (userHour.charAt(0) > 0 && userHour < 10) { // if a single digit like "7" is added as the hour, put a zero in front of it
    userHour = "0" + userHour;
}
const userMinutes = prompt("Please enter the minutes of which you would like the alarm to be set 😄", "00");
